...
Botao padrao
exemplos:
<x-bt-padrao cor="gray" icone="fas fa-arrow-left" texto="Voltar" href="{{ route('veiculos.novos.index') }}"
    atalho="voltar" />
...
$isDynamicHref = $attributes->has('x-bind:href') || $attributes->has(':href');
...
$colorClasses = "text-white bg-{$color}-{$colorValueNormal} hover:bg-{$color}-{$colorValueHover}";
}
...
<div x-data="{ showTooltip: false }" class="relative inline-block group">
    @if ($href || $isDynamicHref)
    <a @if ($href) href="{{ $href }}" @endif @if ($target) target="{{ $target }}" @endif
        @mouseenter="showTooltip = true" @mouseleave="showTooltip = false" {{ $attributes->merge(['class' =>
        $finalClasses]) }}
        >
        @if ($icon)
    ...
    @else
    <button type="{{ $type }}" @mouseenter="if (!{{ $disabled ? 'true' : 'false' }}) showTooltip = true"
        @mouseleave="showTooltip = false" {{ $attributes->merge(['class' => $finalClasses]) }}
        >
        @if ($icon)
        <i class="fas fa-{{ $icon }}"></i>
        @endif
    ...
    @if ($title)
    <div x-show="showTooltip" x-transition.opacity.scale.duration.200ms x-cloak
        ...
        <div class="relative bg-gray-800 text-white text-xs rounded px-3 py-1 shadow-md whitespace-nowrap">
            {{ $title }}
            <div
                class="absolute bottom-[-5px] left-1/2 transform -translate-x-1/2 w-3 h-3 bg-gray-800 rotate-45 z-[-1]">
            </div>
        </div>
    </div>
    @endif
